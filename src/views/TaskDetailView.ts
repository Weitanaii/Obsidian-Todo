...
export class TaskDetailView {
  ...
  openAIRecommendations(items: AIRecommendation[], onRefresh: () => Promise<void>, onAccept: (item: AIRecommendation) => Promise<void>): void {
    this.taskId = null;
    this.historyStack = [];
    this.root.empty();
    ...
    closeBtn.setAttribute("aria-label", t("关闭"));
    closeBtn.setAttribute("title", t("关闭"));
    closeBtn.addEventListener("click", () => { this.close(); this.onClose?.(); });
    const list = this.root.createDiv({ cls: "todo-ai-recommendation-list todo-detail-ai-list" });
    const render = () => {
      list.empty();
      if (!items.length) {
      ...
      for (const item of items) {
        const card = list.createDiv({ cls: "todo-ai-card" });
        ...
        const accept = controls.createEl("button", { cls: "todo-ai-card-action" }); setIcon(accept, "plus");
        accept.title = t("加入"); accept.setAttribute("aria-label", t("加入"));
        accept.addEventListener("click", async () => { accept.disabled = true; await onAccept(item); card.remove(); });
        const dismiss = controls.createEl("button", { cls: "todo-ai-card-action" }); setIcon(dismiss, "x");
        dismiss.title = t("移除推荐"); dismiss.setAttribute("aria-label", t("移除推荐"));
    ...
    setIcon(refresh, "refresh-cw"); refresh.title = t("刷新推荐"); refresh.setAttribute("aria-label", t("刷新推荐"));
    refresh.addEventListener("click", async () => { refresh.toggleClass("is-loading", true); try { await onRefresh(); } finally { refresh.toggleClass("is-loading", false); } });
  }
